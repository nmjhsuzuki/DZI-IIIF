...
def get(s) :
    ...
    n = len(t1) # トークンの数
    # 画像リクエストか画像情報リクエストか
    glo.query = glo.query_mode.INFO if (t1[n - 1].lower() == 'info.json') else glo.query_mode.IMAGE
    if (glo.query == glo.query_mode.INFO): # 画像情報リクエストの場合の処理
        identifier = urllib.parse.unquote(('/'.join(t1[:n - 1]))).strip() if (n >= 2) else ''
        # identifier はそのまま
        ...
        identifier = urllib.parse.unquote(('/'.join(t1[:n - 4]))).strip() if (n >= 5) else ''
